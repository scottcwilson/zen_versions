...
// +----------------------------------------------------------------------+
// $Id: header_php.php 2908 2006-01-29 21:11:47Z wilt $
//
...
//        require($sniffer->phpBB['phpbb_path'] . 'config.php');
        ...
        $sql = "select * from " . $sniffer->phpBB['users_table'] . " where username = '" . zen_db_input($nick) . "'";
//echo $sql;
...
//        require($sniffer->phpBB['phpbb_path'] . 'config.php');
        ...
        $sql = "insert into " . $sniffer->phpBB['users_table'] . "
                ...
                values
                ('" . (int)$user_id . "', '" . zen_db_input($nick) . "', '" . md5($_POST['password']) . "', '" . zen_db_input($email_address) . "', '" . time() ."')";
...
// build the message content
      ...
      if (NEW_SIGNUP_DISCOUNT_COUPON != '' and NEW_SIGNUP_DISCOUNT_COUPON != '0') {
        ...
        $coupon_desc = $db->Execute("select coupon_description from " . TABLE_COUPONS_DESCRIPTION . " where coupon_id = '" . $coupon_id . "' and language_id = '" . (int)$_SESSION['languages_id'] . "'");
        $db->Execute("insert into " . TABLE_COUPON_EMAIL_TRACK . " (coupon_id, customer_id_sent, sent_firstname, emailed_to, date_sent) values ('" . $coupon_id ."', '0', 'Admin', '" . zen_db_input($email_address) . "', now() )");
...
//        $email_text .= EMAIL_COUPON_INCENTIVE_HEADER .
      ...
      if (NEW_SIGNUP_GIFT_VOUCHER_AMOUNT > 0) {
        ...
        $db->Execute("insert into " . TABLE_COUPON_EMAIL_TRACK . " (coupon_id, customer_id_sent, sent_firstname, emailed_to, date_sent) values ('" . $insert_id ."', '0', 'Admin', '" . zen_db_input($email_address) . "', now() )");
