...
// update customers_referral with $_SESSION['gv_id']
  if ($_SESSION['cc_id']) {
    $discount_coupon_query = "select coupon_code
                 from " . TABLE_COUPONS . "
                 where coupon_id = '" . zen_db_input($_SESSION['cc_id']) . "'";

    $discount_coupon = $db->Execute($discount_coupon_query);

    $customers_referral_query = "select customers_referral from " . TABLE_CUSTOMERS . " where customers_id='" . (int)$_SESSION['customer_id'] . "'";
...
// only use discount coupon if set by coupon
    if ($customers_referral->fields['customers_referral'] == '' and CUSTOMERS_REFERRAL_STATUS == 1) {
      $db->Execute("update " . TABLE_CUSTOMERS . " set customers_referral='" . $discount_coupon->fields['coupon_code'] . "' where customers_id='" . (int)$_SESSION['customer_id'] . "'");
    } else {
