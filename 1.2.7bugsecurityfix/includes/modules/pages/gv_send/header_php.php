...
// +----------------------------------------------------------------------+
// $Id: header_php.php 2911 2006-01-30 20:10:11Z wilt $
//
...
//die($currencies->value($_POST['amount'], true, $_SESSION['currency']));
    ...
    if ($new_amount < 0) {
      ...
      $gv_query="update " . TABLE_COUPON_GV_CUSTOMER . "
                 set amount = '" .  zen_db_input($new_amount) . "'
                 where customer_id = '" . (int)$_SESSION['customer_id'] . "'";
      ...
      $gv_query="insert into " . TABLE_COUPONS . "
                 ...
                 values ('G', '" . zen_db_input($id1) . "', NOW(), '" . $currencies->value($_POST['amount'], true, DEFAULT_CURRENCY) . "')";
      ...
      $gv_query="insert into " . TABLE_COUPON_EMAIL_TRACK . "
                 ...
                 values ('" . (int)$insert_id . "' ,'" . (int)$_SESSION['customer_id'] . "', '" .
                         zen_db_input($gv_customer->fields['customers_firstname']) . "', '" .
                         zen_db_input($gv_customer->fields['customers_lastname']) . "', '" .
                         zen_db_input(zen_db_prepare_input($_POST['email'])) . "', now())";
      ...
      $gv_email = STORE_NAME . "\n" .
		...
		$html_msg['EMAIL_GV_AMOUNT'] =  $currencies->format(zen_db_prepare_input($_POST['amount']), false);
		$html_msg['EMAIL_GV_FROM'] =  sprintf(EMAIL_GV_FROM, zen_db_prepare_input($_POST['send_name'])) ;
      ...
      if (isset($_POST['message'])) {
		...
		$html_msg['EMAIL_GV_SEND_TO'] = '<tt>'.sprintf(EMAIL_GV_SEND_TO, zen_db_prepare_input($_POST['to_name'])). '</tt><br />';
        }
        $gv_email .= stripslashes($_POST['message']) . "\n\n";
        $gv_email .= EMAIL_SEPARATOR . "\n\n";
		$html_msg['EMAIL_MESSAGE_HTML'] = stripslashes(zen_db_prepare_input($_POST['message']));
      }
      ...
      $gv_email_subject = sprintf(EMAIL_GV_TEXT_SUBJECT, zen_db_prepare_input($_POST['send_name']));
...
// send the email
      zen_mail('', zen_db_prepare_input($_POST['email']), $gv_email_subject, nl2br($gv_email), STORE_NAME, EMAIL_FROM, $html_msg,'gv_send');
