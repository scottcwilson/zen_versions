...
// +----------------------------------------------------------------------+
// $Id: header_php.php 2902 2006-01-27 06:47:23Z drbyte $
//
...
// 1.2.7 checks
  ...
  $result = $db_test->Execute($sql);
  if (ZC_UPG_DEBUG==true) echo "127a-configkey_check=" . $result->fields['configuration_title'] . '<br>';
  if  ($result->fields['configuration_description'] == 'Automatically check to see if a new version of Zen Cart is available. Enabling this can sometimes slow down the loading of Admin pages. (Displayed on main Index page after login, and Server Info page.)') {
    $got_v1_2_7a = true;
    if (ZC_UPG_DEBUG==true) echo '<br>Got 1.2.7a<br>';
  }
  //check #2
  $sql = "select configuration_description from " . DB_PREFIX . "configuration where configuration_key='SHOW_SHOPPING_CART_BOX_STATUS'";
  $result = $db_test->Execute($sql);
  if (ZC_UPG_DEBUG==true) echo "127b-configkey_check=" . $result->fields['configuration_title'] . '<br>';
  if  ($result->fields['configuration_description'] == 'Shopping Cart Shows<br />0= Always<br />1= Only when full<br />2= Only when full but not when viewing the Shopping Cart') {
    $got_v1_2_7b = true;
    if (ZC_UPG_DEBUG==true) echo '<br>Got 1.2.7b<br>';
  }
...
if (empty($path_trans)) {
 ...
 $status_check[] = array('Importance' => 'Info', 'Title' => LABEL_PATH_TRANLSATED, 'Status' => $path_trans_display, 'Class' => 'NA', 'HelpURL' =>'', 'HelpLabel'=>'');
  ...
  $status_check[] = array('Importance' => 'Optional', 'Title' => LABEL_PHP_OS, 'Status' => PHP_OS, 'Class' => 'NA', 'HelpURL' =>$err_code, 'HelpLabel'=>$err_text);
 ...
 $status_check[] = array('Importance' => 'Info', 'Title' => LABEL_REGISTER_GLOBALS, 'Status' => $register_globals, 'Class' => $this_class, 'HelpURL' =>ERROR_CODE_REGISTER_GLOBALS_ON, 'HelpLabel'=>ERROR_TEXT_REGISTER_GLOBALS_ON);
  ...
  //DB Privileges
if (false) { // DISABLED THIS BLOCK FOR NOW....
...
$status_check[] = array('Importance' => 'Critical', 'Title' => LABEL_SAFE_MODE, 'Status' => $safe_mode, 'Class' => ($safe_mode==OFF) ? 'OK' : 'FAIL', 'HelpURL' =>ERROR_CODE_SAFE_MODE_ON, 'HelpLabel'=>ERROR_TEXT_SAFE_MODE_ON);
  ...
  $this_class = 'OK';
  $path_found_in_restrictions_list = false;
  if ($open_basedir=@ini_get('open_basedir')) {
    ...
    foreach($basedir_check_array as $basedir_check) {
      if (strstr($dir_fs_www_root, $basedir_check)) $path_found_in_restrictions_list=true;
    }
    if ($path_found_in_restrictions_list == false) $this_class = 'FAIL';
  }
