...
// Alias function for Store configuration values in the Administration Tool
  function zen_cfg_pull_down_country_list($country_id) {
    return zen_draw_pull_down_menu('configuration_value', zen_get_countries(), $country_id);
  }


  function zen_cfg_pull_down_country_list_none($country_id) {
    $country_array = zen_get_countries('None');
    return zen_draw_pull_down_menu('configuration_value', $country_array, $country_id);
  }


  function zen_cfg_pull_down_zone_list($zone_id) {
    return zen_draw_pull_down_menu('configuration_value', zen_get_country_zones(STORE_COUNTRY), $zone_id);
  }
...
// Function to read in text area in admin
 function zen_cfg_textarea($text) {
    return zen_draw_textarea_field('configuration_value', false, 60, 5, $text);
  }


////
// Function to read in text area in admin
 function zen_cfg_textarea_small($text) {
    return zen_draw_textarea_field('configuration_value', false, 35, 1, $text);
  }
...
// example: zen_set_field_length(TABLE_CATEGORIES_DESCRIPTION, 'categories_name')
  function zen_set_field_length($tbl, $fld, $max=50) {
  ...
  function zen_draw_products_pull_down_categories($name, $parameters = '', $exclude = '', $show_id = false, $show_parent = false) {
    ...
    while (!$categories->EOF) {
      if (!in_array($categories->fields['categories_id'], $exclude)) {
        if ($show_parent == true) {
          $parent = zen_get_parent_category_name($categories->fields['categories_id']);
          if ($parent != '') {
            $parent = ' : in ' . $parent;
          }
        } else {
...
// Count how many subcategories exist in a category
// TABLES: categories
  function zen_get_parent_category_name($categories_id) {
    global $db;
...
// master category selection
  function zen_get_master_categories_pulldown($product_id) {
    ...
    while (!$master_categories_query->EOF) {
      $master_category_array[] = array('id' => $master_categories_query->fields['categories_id'], 'text' => $master_categories_query->fields['categories_name']);
