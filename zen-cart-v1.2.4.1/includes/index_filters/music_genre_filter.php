...
// +----------------------------------------------------------------------+
// $Id: music_genre_filter.php 277 2004-09-10 23:03:52Z wilt $
//
...
// We are asked to show only a specific category
        $listing_sql = "select " . $select_column_list . " p.products_id, p.products_price, p.products_tax_class_id, if(s.status = '1', s.specials_new_products_price, NULL) AS specials_new_products_price, IF(s.status = '1', s.specials_new_products_price, p.products_price) as final_price, p.products_sort_order from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCT_MUSIC_EXTRA . " pme, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c, " . TABLE_MUSIC_GENRE . " m left join " . TABLE_SPECIALS . " s on pme.products_id = s.products_id where  m.music_genre_id = '" . (int)$_GET['music_genre_id'] . "' and p.products_id = pme.products_id and p.products_status = '1' and pme.music_genre_id = '" . (int)$_GET['music_genre_id'] . "' and pme.products_id = p2c.products_id and pd.products_id = p2c.products_id and pd.language_id = '" . (int)$_SESSION['languages_id'] . "' and p2c.categories_id = '" . (int)$_GET['filter_id'] . "'";
      } else {
// We show them all
        $listing_sql = "select " . $select_column_list . " pme.products_id, p.products_price, p.products_tax_class_id, IF(s.status = '1', s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status = '1', s.specials_new_products_price, p.products_price) as final_price, p.products_sort_order from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCT_MUSIC_EXTRA . " pme, " . TABLE_MUSIC_GENRE . " m left join " . TABLE_SPECIALS . " s on pme.products_id = s.products_id where  m.music_genre_id = '" . (int)$_GET['music_genre_id'] . "' and p.products_id = pme.products_id and p.products_status = '1' and  pd.products_id = pme.products_id and pd.language_id = '" . (int)$_SESSION['languages_id'] . "' and pme.music_genre_id = '" . (int)$_GET['music_genre_id'] . "'";
