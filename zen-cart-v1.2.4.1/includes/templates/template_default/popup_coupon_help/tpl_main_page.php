...
// +----------------------------------------------------------------------+
// $Id: tpl_main_page.php 743 2004-12-08 22:24:00Z birdbrain $
//
?>
<body onload="resize();">
...
<?php
  ...
  while (!$get_result->EOF) {
    ...
    $result = $db->Execute("SELECT * FROM " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd WHERE c.categories_id = cd.categories_id and cd.language_id = '" . $languages_id . "' and c.categories_id='" . $get_result->fields['category_id'] . "'");
  ...
  while (!$get_result->EOF) {
    if ($get_result->fields['coupon_restrict'] == 'N') {
      ...
      $result = $db->Execute("SELECT * FROM " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd WHERE p.products_id = pd.products_id and pd.language_id = '" . $languages_id . "'and p.products_id = '" . $get_result->fields['product_id'] . "'");
