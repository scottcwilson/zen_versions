...
//die($currencies->value($_POST['amount'], true, $_SESSION['currency']));
    ...
    if ($new_amount < 0) {
	  ...
	  $html_msg['GV_REDEEM_URL'] = '<a href="'.zen_href_link(FILENAME_GV_REDEEM, 'gv_no=' . $id1 ).'">'.EMAIL_GV_LINK.'</a>';
	  $html_msg['GV_REDEEM_CODE'] = $id1;

      $gv_email .= sprintf(EMAIL_GV_REDEEM, $id1) . "\n\n";
      $gv_email .= EMAIL_GV_LINK . ' ' . zen_href_link(FILENAME_GV_REDEEM, 'gv_no=' . $id1);;
      $gv_email .= "\n\n";
